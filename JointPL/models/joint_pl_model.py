...
from .Hourglass2_LB import HourglassNet2_LB
from .base_model import BaseModel
from collections import OrderedDict
from .line_loss import lcmap_head, lcoff_head, lleng_head, angle_head, _loss, _loss2
...
class KeypointNetwithIOLoss(BaseModel):
    """
    ...
    default_conf = {
        ...
        'keypoint_net_learning_rate': 0.0004,
        'with_io': True,
        'use_color': True,
        'do_upsample': True,
        'do_cross': True,
    ...
    required_data_keys = ['image', 'image_aug', 'idx', 'homography']

    def _init(self, conf):

        # super().__init__()

        self.keypoint_loss_weight = conf.keypoint_loss_weight
        self.descriptor_loss_weight = conf.descriptor_loss_weight
        self.score_loss_weight = conf.score_loss_weight
        self.keypoint_net_learning_rate = conf.keypoint_net_learning_rate
        self.optim_params = []
        ...
        self.use_color = conf.use_color
        self.descriptor_loss = conf.descriptor_loss
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        # Initialize KeypointNet

        self.keypoint_net = HourglassNet2_LB()

        self.keypoint_net = self.keypoint_net.to(device)

        self.add_optimizer_params('KeypointNet', self.keypoint_net.parameters(), self.keypoint_net_learning_rate)

        self.with_io = conf.with_io
        self.io_net = None
        if self.with_io:
            self.io_net = InlierNet(blocks=4)
            self.io_net = self.io_net.to(device)
            self.add_optimizer_params('InlierNet', self.io_net.parameters(), self.keypoint_net_learning_rate)
    ...
    def _forward(self, data):
        ...
        if self.training or not self.training:
            ...
            input_img_aug = input_img_aug.clone()

            # Get network outputs
            source_score, source_uv_pred, source_feat, _, _ = self.keypoint_net(input_img_aug)
            target_score, target_uv_pred, target_feat, outputs, feature = self.keypoint_net(input_img)
            ...
            for stack, output in enumerate(outputs):
                ...
                #if stack == 0 and data["do_evaluation"]:
                #    result["heatmaps"] = heatmap

            result["losses"] = losses
            result["accuracy"] = accuracy

            loss_line = _loss2(result)
            loss_total = 25*loss_line + loss_point

        return {'total': loss_total}, {'point': loss_point}, {'line': loss_line}, {'recall': torch.tensor([recall_2d])}

    def loss(self, pred, data):
        loss, _, _, _ = self._forward(data)
        return loss

    def metrics(self, pred, data):
        _, _, _, recall = self._forward(data)
        return recall
