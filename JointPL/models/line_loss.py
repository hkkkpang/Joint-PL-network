...
def lcmap_head(output, target):
    ...
    head_off = [2, 4, 5, 6]
    s = 0 if offidx == 0 else head_off[offidx - 1]

    pred = output[s: head_off[offidx]].reshape(2, batch, row, col)
...
def lcoff_head(output, target, mask):
    name = 'lcoff'
...
def lleng_head(output, target, mask):
    name = 'lleng'
    ...
    pred = output[s: head_off[offidx]].reshape(batch, row, col)
...
def angle_head(output, target, mask):
    name = 'angle'
